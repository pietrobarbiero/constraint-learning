...
def booleanConstraint(weights,bias):
	...
	rel_pos = [[np.nonzero(weights[j][i]) for i in range(h[j])] for j in range(num_layers)]
	...
	for j in range(num_layers):
		...
		print(form_labels)
		for i in range(h[j]):
			formula.append("("+calc_form(X,y[j][i],form_labels,rel_pos[j][i][0])+")")
		form_labels=formula
	return formula
